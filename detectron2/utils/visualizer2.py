...
def _create_text_labels(classes, scores, class_names, ID, indice, ave_scores):
    """
    Args:
    ...
    if indice is not None and class_names is not None and len(class_names) > 1:
        lables_ac = [class_names[i] for i in indice]
    if scores is not None:
        if labels is None:
            sco = [np.round(num*100) for num in scores.numpy()]
             
            sca = [np.round(i* 100) for i in ave_scores]
            labels = ["ID:{}\n{} \nID_AC:{}-- {}".format(t, s, i, sa) for s, t, i, sa in zip(sco, ID, indice, sca)]
            ...
            #print(sca)
            #sco = scores.tolist()
            #labels = ["{},ID:{}-- {:.0f}%".format(l, t, s * 100) for l, s, t in zip(labels, sco, ID)]
            labels = ["ID:{}\nInstant:{}-- {} \nAccum:{}-- {}".format(t, l, s, i, sa) for l, s, t, i, sa in zip(labels, sco, ID, lables_ac, sca)]
    return labels
...
class Visualizer:
    def __init__(self, img_rgb, metadata, scale=1.0, instance_mode=ColorMode.IMAGE):
        ...
        self._default_font_size = max(
            np.sqrt(self.output.height * self.output.width) // 70, 10 // scale
        )
        self._instance_mode = instance_mode

    def draw_instance_predictions(self, predictions, id, indice, ave_scores):
        """
        ...
        labels = _create_text_labels(classes, scores, self.metadata.get("thing_classes", None), id, indice, ave_scores)
        ...
        *,
        ...
        for i in range(num_instances):
            ...
            if labels is not None:
                ...
                if boxes is not None:
                    x0, y0, x1, y1 = boxes[i]
                    text_pos = (x0, y0-200)  # if drawing boxes, put text on the box corner.
                    horiz_align = "left"
                elif masks is not None:
